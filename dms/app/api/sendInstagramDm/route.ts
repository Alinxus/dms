// /app/api/sendInstagramDms/route.ts
...
export async function POST(req: NextRequest) {
  ...
  // Fetch usernames from CSV or from the Google Sheets
  let usernames: string[] = [];
  if (csvFilePath) {
    usernames = await parseCsv(csvFilePath);
  } else {
    usernames = await readAllDataFromSheet();
  }
  ...
  try {
    ...
    await browser.close();
    return NextResponse.json({ success: true, message: 'Instagram DMs sent successfully' });
